...
import { fetchBlogs, IBlog, IInitialStateAllBlogs } from "../../redux/reducers/Blogs/getAllBlogs";
...
const Blogs = () => {
  ...
  React.useEffect(() => {
    dispatch(fetchBlogs({ limit })).then((data) => setBlogs(data.payload.blogs));
          ...
          <div className="grid gap-x-5 gap-y-16 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-1 sm:grid-cols-1 tracking-wider">
            {filteredBlogs?.length === 0 ? (
              <p className="text-center w-full col-span-3">
                <span className="text-[#fff] text-3xl block">Üzür istəyirik</span>
                <span className="text-[#8D8D8D] text-3xl block">axtarışa uyğun nəticə tapılmadı!</span>
                <button
